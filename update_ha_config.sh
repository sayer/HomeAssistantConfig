...
main() {
  ...
  if pushd "$REPO_DIR" >/dev/null 2>&1; then
    HA_CHECK_OUTPUT=$(HOME="$HA_CLI_HOME" HASS_CONFIG="$REPO_DIR" "${HA_CHECK_CMD[@]}" 2>&1)
    HA_CHECK_STATUS=$?
    popd >/dev/null 2>&1
  else
    HA_CHECK_OUTPUT=$(HOME="$HA_CLI_HOME" HASS_CONFIG="$REPO_DIR" "${HA_CHECK_CMD[@]}" 2>&1)
  ...
  if [ $HA_CHECK_STATUS -ne 0 ] && printf '%s' "$HA_CHECK_OUTPUT" | grep -q "configuration.yaml not found"; then
    HA_CHECK_OUTPUT=$(cd "$REPO_DIR" && HOME="$HA_CLI_HOME" HASS_CONFIG="$REPO_DIR" ha core check --config "$REPO_DIR" 2>&1)
    HA_CHECK_STATUS=$?
  fi
